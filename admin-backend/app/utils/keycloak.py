...
# # ) -> str:
# #     """
# #     Create a user in Keycloak with the specified role and custom attributes.
...
# #         logger.info(f"Created Keycloak user with email: {email}, user_id: {user_id}")
...
# #             logger.error(f"Failed to assign '{role_name}' role for email {email}: {str(role_e)}")
...
#             # update_data["username"] = email  # Update username to match email  # Do NOT update username, as it's read-only
...
#             logger.info(f"Updated Keycloak user details for user_id: {user_id}")
