...
# # Helper function to convert Student model to dict
# def student_to_dict(student: Student) -> dict:
#     return {c: getattr(student, c) for c in student._defined_columns}
...
#         logger.exception(f"Pydantic validation error: {str(e)}")
#         raise HTTPException(status_code=422, detail=f"Invalid input data: {str(e)}")
#     except NoHostAvailable as e:
#         logger.exception(f"Cassandra connection error: {str(e)}")
