...
# Helper function to convert Teacher model to dict
def teacher_to_dict(teacher: Teacher) -> dict:
    return {c: getattr(teacher, c) for c in teacher._defined_columns}

def get_teacher(teacher_id: UUID) -> Optional[TeacherInDB]:
    try:
        teacher = Teacher.objects(id=teacher_id).first()
        return TeacherInDB(**teacher_to_dict(teacher)) if teacher else None
...
def get_teachers(skip: int = 0, limit: int = 100) -> List[TeacherInDB]:
    try:
        ...
        return [TeacherInDB(**teacher_to_dict(t)) for t in teachers]
    except Exception as e:
...
def create_teacher(teacher: TeacherCreate) -> TeacherInDB:
    try:
        ...
        return TeacherInDB(**teacher_to_dict(new_teacher))
    except Exception as e:
...
def update_teacher(teacher_id: UUID, teacher: TeacherUpdate) -> Optional[TeacherInDB]:
    try:
        ...
        return TeacherInDB(**teacher_to_dict(existing_teacher))
    except Exception as e:
