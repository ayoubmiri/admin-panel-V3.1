...
# Helper function to convert Module model to dict
def module_to_dict(module: Module) -> dict:
    return {c: getattr(module, c) for c in module._defined_columns}
...
def get_modules(skip: int = 0, limit: int = 100) -> List[ModuleInDB]:
    try:
        ...
        return [ModuleInDB(**module_to_dict(m)) for m in modules]
    except Exception as e:
...
def create_module(module: ModuleCreate) -> ModuleInDB:
    try:
        ...
        return ModuleInDB(**module_to_dict(new_module))
    except Exception as e:
...
def update_module(module_id: UUID, update: ModuleUpdate) -> Optional[ModuleInDB]:
    try:
        ...
        existing.save()
        return ModuleInDB(**module_to_dict(existing))
    except Exception as e:
