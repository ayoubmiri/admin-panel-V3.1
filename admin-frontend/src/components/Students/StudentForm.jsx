...
//               </label>
//               <select
//                 className={`shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline ${errors.student_type ? 'border-red-500' : ''}`}
//                 id="student_type"
//                 name="student_type"
...
//                 <option value="Temps Aménagé">Temps Aménagé</option>
//                 <option value="Temps Normale">Temps Normale</option>
//               </select>
//               {errors.student_type && <p className="text-red-500 text-xs italic">{errors.student_type}</p>}
//             </div>
...
const StudentForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.student_id.trim()) newErrors.student_id = 'Identifiant étudiant requis';
    ...
    if (!formData.student_type) newErrors.student_type = 'Type d’étudiant requis';
    if (!formData.year) newErrors.year = 'Année requise';
    if (formData.date_of_birth && isNaN(new Date(formData.date_of_birth).getTime()))
      newErrors.date_of_birth = 'Date de naissance invalide';
    if (formData.class_id && !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(formData.class_id))
      newErrors.class_id = 'L’identifiant de la classe doit être un UUID valide';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        await updateStudent(id, payload);
        setMessage({ text: 'Étudiant mis à jour avec succès !', type: 'success' });
      } else {
        await createStudent(payload);
        setMessage({ text: 'Étudiant créé avec succès !', type: 'success' });
        setFormData({
          student_id: '',
          first_name: '',
          last_name: '',
      ...
      console.error('Error saving student:', error);
      let errorMessage = 'Échec de l’enregistrement de l’étudiant. Veuillez vérifier vos données et réessayer.';
      if (error.response?.status === 422 && error.response.data?.detail) {
        const details = Array.isArray(error.response.data.detail)
          ? error.response.data.detail.map(err => `${err.loc.join('.')}: ${err.msg}`).join('; ')
          : error.response.data.detail;
  ...
  return (
    <div className="bg-white rounded-lg shadow mb-6">
      <div className="p-4 border-b">
        <h2 className="font-semibold text-lg">{id ? 'Modifier Étudiant' : 'Ajouter Nouvel Étudiant'}</h2>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select
                ...
                onChange={handleChange}
              >
                <option value="">Sélectionner le type d’étudiant</option>
                <option value="Temps Aménagé">Temps Aménagé</option>
                <option value="Temps Normale">Temps Normale</option>
              </select>
              {errors.student_type && <p className="text-red-500 text-xs italic">{errors.student_type}</p>}
