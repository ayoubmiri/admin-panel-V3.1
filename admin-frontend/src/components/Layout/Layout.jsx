...
const Layout = () => {
  ...
  const activeTab = location.pathname.split('/')[1] || 'tableau-de-bord';
        ...
        <div className="p-4">
          ...
          <nav className="space-y-1">
            {navItems.map((item, index) => {
              const itemPath = item.path.split('/')[1] || 'tableau-de-bord';
          ...
          <div className="flex items-center justify-between p-4 h-full">
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-semibold text-gray-800 capitalize">
                {navItems.find(item => item.path === `/${activeTab.replace('tableau-de-bord', '')}`)?.text?.toLowerCase().replace('é', 'e') || 'Tableau de bord'}
              </h1>
            </div>
