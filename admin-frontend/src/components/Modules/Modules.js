...
import { FaSearch, FaPlus, FaEye, FaEdit, FaTrash } from 'react-icons/fa';
import { getModules } from '../../services/moduleService';
import { getFilieres } from '../../services/filiereService'; // Import getFilieres

const Module = () => {
  const [modules, setModules] = useState([]);
  const [filieres, setFilieres] = useState([]); // State for filieres
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch modules
        const moduleData = await getModules(searchTerm);
        // Fetch filieres
        const filiereData = await getFilieres();
        const validFilieres = Array.isArray(filiereData)
          ? filiereData.filter(filiere => /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(filiere.id))
          : [];
        setFilieres(validFilieres);

        // Map filiere names to modules
        const modulesWithFiliere = moduleData.map(module => ({
          ...module,
          filiere: validFilieres.find(filiere => filiere.id === module.filiere_id) || { name: 'Not assigned' }
        }));

        setModules(modulesWithFiliere);
      } catch (error) {
  ...
  return (
    <div className="bg-white rounded-lg shadow mb-6">
      <div className="p-4 border-b flex flex-col md:flex-row md:justify-between md:items-center">
        ...
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-2">
          <div className="relative">
            <input
              type="text"
              placeholder="Search modules or filieres..."
              className="pl-10 pr-4 py-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 w-full"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
      ...
      {loading ? (
        ...
        <div className="p-8 text-center">No modules found.</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
